...
// ── Block Modal ──
function BlockModal({ isOpen, onClose, data, onSuccess }) {
  ...
  useEffect(() => {
    if (data) {
      setFormData({
        country: data.country || "India / भारत",
        state: data.state || "",
        district: data.district || "",
        name: data.name || "",
        nameHi: data.nameHi || ""
      });
      if (data.state && indianDistricts[data.state]) {
        setDistricts(indianDistricts[data.state]);
      }
    } else {
      setFormData({
        country: "India / भारत",
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        const rows = await Promise.all(lines.slice(1).map(async (line) => {
          ...
          const country = countryEn.toLowerCase() === "india" ? "India / भारत" : countryEn;
          ...
          return { country, state, district, name: nameEn, nameHi: nameHi };
        }));
  ...
  const downloadTemplate = () => {
    const headers = "Country,State,District,Block Name\n";
    const sampleRows = "India,Bihar,Patna,Sadar\n";
    const blob = new Blob([headers + sampleRows], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'block_upload_template.csv';
    a.click();
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (activeTab === "bulk") {
        ...
        const res = await locationAPI.bulkCreateBlocks(bulkPreview);
        if (res && !res.error) {
          onSuccess(); 
          onClose();
        } else {
      ...
      let res;
      if (data?._id) {
        res = await locationAPI.updateBlock(data._id, dataToSave);
      } else {
        res = await locationAPI.createBlock(dataToSave);
      }
      ...
      if (res && !res.error) {
        ...
        alert(res?.error || "Failed to save block");
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200 my-auto">
        <div className="flex items-center justify-between px-6 py-4 bg-[#3AB000]">
          <h2 className="text-white font-bold text-base flex items-center gap-2">
            {data ? "✏️ Edit Block" : "🏢 Add New Block"}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {activeTab === "single" ? (
            <div className="grid grid-cols-1 gap-5">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-gray-500 uppercase tracking-wider block">Block Name (English)</label>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between p-3 bg-blue-50 border border-blue-100 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-blue-700">Need a template?</span>
                </div>
                <button
                  type="button"
              ...
              >
                ...
                <p className="text-xs text-gray-400 mt-1">Accepts .csv files (max 1000 rows)</p>
              ...
              {bulkPreview.length > 0 && (
                <div className="rounded-lg border border-gray-100 overflow-hidden">
                  <div className="bg-gray-50 px-4 py-2 border-b border-gray-100 flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">File Preview</span>
                    <span className="text-[10px] font-bold text-green-600">{bulkPreview.length} Records Detected</span>
                  </div>
                  <div className="max-h-40 overflow-y-auto">
                    <table className="w-full text-left text-[11px]">
                      <thead className="sticky top-0 bg-white">
                        <tr className="text-gray-400 border-b border-gray-50">
                          <th className="px-3 py-2 font-bold uppercase">State</th>
                          <th className="px-3 py-2 font-bold uppercase">District</th>
                          <th className="px-3 py-2 font-bold uppercase">Block Name</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {bulkPreview.slice(0, 5).map((row, i) => (
                          <tr key={i}>
                            ...
                            <td className="px-3 py-2 font-bold text-[#3AB000]">
                              {row.name} / {row.nameHi}
                            </td>
                          </tr>
                        ))}
                    ...
                    {bulkPreview.length > 5 && (
                      <div className="px-3 py-2 bg-white text-center border-t border-gray-50 text-gray-400 font-medium italic">
                        And {bulkPreview.length - 5} more rows...
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="px-6 py-4 bg-gray-50 -mx-6 -mb-6 border-t border-gray-100 flex items-center justify-end gap-3 mt-6">
            ...
            >
              {isSaving ? "Saving..." : <>{activeTab === "bulk" ? <Upload size={16}/> : <Save size={16} />} {activeTab === "bulk" ? "Import All Blocks" : "Save Block"}</>}
            </button>
          </div>
        </form>
      </div>
...
const BlockManagement = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure? This will also delete associated Panchayats.")) {
      try {
        const res = await locationAPI.deleteBlock(id);
  ...
  return (
    <DashboardLayout activePath="/location/blocks">
      {isLoading && <LoadingOverlay message="Fetching Blocks..." />}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#3AB000]">
                ...
                <th className="px-6 py-3 font-bold text-black text-sm text-center">Block Name / प्रखंड</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-400 italic">No blocks found matching filters.</td>
                </tr>
              ) : (
                filteredData.map((block, idx) => (
                  <tr key={block._id} className={`${selectedIds.includes(block._id) ? 'bg-[#e8f5e2]' : 'hover:bg-[#e8f5e2]'} transition-colors group`}>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="text-sm font-bold text-gray-800">{block.name} / {block.nameHi}</span>
                    </td>
            ...
            <div key={block._id} className={`bg-white rounded-lg border ${selectedIds.includes(block._id) ? 'border-[#3AB000] ring-1 ring-[#3AB000]' : 'border-gray-200'} p-4 shadow-sm transition-all`}>
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-1">
                  <button onClick={() => { setSelectedBlock(block); setIsModalOpen(true); }} className="p-1.5 text-blue-500 bg-blue-50 rounded"><Edit size={14}/></button>
                  <button onClick={() => handleDelete(block._id)} className="p-1.5 text-red-500 bg-red-50 rounded"><Trash2 size={14}/></button>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[11px] border-t border-gray-50 pt-3">
                <div className="flex flex-col">
                  <span className="text-gray-400 uppercase font-black tracking-tighter">State</span>
                  <span className="font-bold text-gray-700">{block.state.split(' / ')[0]}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-gray-400 uppercase font-black tracking-tighter">District</span>
                  <span className="font-bold text-gray-700">{block.district.split(' / ')[0]}</span>
                </div>
              </div>
            </div>
          ))}
